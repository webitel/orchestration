...
case "$1" in
    "backup")
        ...
        docker exec -it elasticsearch2 curl -XPUT -d '{"type": "fs","settings": {"location": "es"}}' -H 'Content-Type: application/json' localhost:9200/_snapshot/es
        docker exec -it elasticsearch2 curl -XPUT "localhost:9200/_snapshot/es/snapshot?wait_for_completion=true"
        ...
        docker exec -it elasticsearch2 curl -XDELETE localhost:9200/_snapshot/es
